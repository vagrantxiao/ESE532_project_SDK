...
void Load_data(unsigned char * Data)
{
  ...
  Check_error(Result != FR_OK, "Could not open input file1.");

  Result = f_read(&File, Data, Size, &Bytes_read);
  Check_error(Result != FR_OK || Bytes_read != Size, "Could not read input file2.");

  Check_error(f_close(&File) != FR_OK, "Could not close input file3.");
}
...
void Store_data(const char * Filename, unsigned char * Data, unsigned int Size)
{
  ...
  Check_error(Result != FR_OK, "Could not open output file1.");

  Result = f_write(&File, Data, Size, &Bytes_written);
  Check_error(Result != FR_OK || Bytes_written != Size, "Could not write output file2.");

  Check_error(f_close(&File) != FR_OK, "Could not close output file3.");
...
#else
#define RX_INTR_ID		XPAR_FABRIC_AXI_DMA_0_MM2S_INTROUT_INTR
#define TX_INTR_ID		XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR
#endif
...
static int SendPacket(XAxiDma * AxiDmaInstPtr, unsigned char * Data_temp);
...
int main(void)
{
	...
	/* Send a packet */
	Status = SendPacket(&AxiDma, Data_temp);
	if (Status != XST_SUCCESS) {
...
Done:
	...
	//time1 += sds_clock_counter() - Clock;

	printf("Hardware needs %llu cycles.\n", time1);
	Store_data("Output.tar", Data_temp, com_file_len);
...
static int SendPacket(XAxiDma * AxiDmaInstPtr, unsigned char * Data_temp)
{
	...
	for(Index = 0; Index < MAX_PKT_LEN * NUMBER_OF_BDS_TO_TRANSFER;
		...
		TxPacket[Index] = Data_temp[Index];
